...
def admin_edit(executor, tour_id, tour_max):
    btns = []
    for user in executor:
        btns.append([InlineKeyboardButton(text=f'{user["executor"]}', callback_data=f'_'),
                InlineKeyboardButton(text='➖', callback_data=f'kount_-_{user["app_id"]}_{user["client_count"]}_{tour_max}_/{tour_id}'),
